...
export async function installMarketplacePlugin(manifest = {}, marketEntry = {}) {
    ...
    return updatePluginRecord(pluginId, (currentRecord) => ({
        ...
        sourceLabel: normalizeString(manifest.source?.sourceLabel || marketEntry.displayName || manifest.displayName),
        manifestUrl: normalizeString(manifest.source?.manifestUrl || marketEntry.manifestUrl),
        entryUrl: normalizeString(manifest.script?.entry),
        exportName: normalizeString(manifest.script?.exportName, 'default'),
