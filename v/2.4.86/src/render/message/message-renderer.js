...
function getMessageRenderContext() {
    ...
    return {
        previewModal,
        previewImage: previewModal?.querySelector?.('img') || null,
        messageInput: document.getElementById('message-input'),
    };
}

function createMessageImageTagNode(base64Data, renderContext) {
    if (!base64Data) return null;

    const { previewModal, previewImage, messageInput } = renderContext || {};
    const imageTag = createImageTag({
        base64Data,
        config: {
            onImageClick: (nextBase64Data) => {
                if (!previewModal || !previewImage) return;
                showImagePreview({
                    base64Data: nextBase64Data,
                    config: {
                        previewModal,
                        previewImage
                    }
...
function resolveMessageRenderData(text) {
    ...
    const plainTextContent = Array.isArray(rawContent)
        ...
        : String(rawContent ?? '');
    const renderContext = getMessageRenderContext();
    ...
    if (Array.isArray(rawContent)) {
        rawContent.forEach((item) => {
            if (item?.type === 'text') {
                ...
                const imageTag = createMessageImageTagNode(item?.image_url?.url, renderContext);
                if (imageTag) {
    ...
    return {
        ...
        messageText,
        imageTagNodes,
        reasoningContent: typeof text === 'string' ? null : text?.reasoning_content || null,
        seedId: typeof text === 'string' ? null : text?.seedId || null,
        isSeedManaged: typeof text === 'string' ? false : !!text?.seedLocaleBound,
        renderContext,
    };
}

function applyMessageMetadata(messageElement, { plainTextContent, seedId, isSeedManaged }) {
    messageElement.setAttribute('data-original-text', plainTextContent);

    if (seedId) {
        messageElement.dataset.seedId = seedId;
    } else {
        delete messageElement.dataset.seedId;
    }

    if (isSeedManaged) {
        messageElement.dataset.seedManaged = '1';
    } else {
        delete messageElement.dataset.seedManaged;
    }
}

function createMainContentNode({ messageText, imageTagNodes, sender }) {
...
async function finalizeRenderedMessage(messageElement, { plainTextContent, reasoningContent, renderContext }) {
    if (textMayContainMath(plainTextContent) || textMayContainMath(reasoningContent)) {
        try {
            await renderMathInElement(messageElement);
        } catch (err) {
    ...
    messageElement.querySelectorAll('.image-tag').forEach((tag) => {
        if (tag.dataset.cerebrBuilt === '1') return;
        const img = tag.querySelector('img');
        const base64Data = tag.getAttribute('data-image');
        if (img && base64Data && renderContext?.previewModal && renderContext?.previewImage) {
            img.addEventListener('click', (e) => {
                ...
                showImagePreview({
                    base64Data,
                    config: {
                        previewModal: renderContext.previewModal,
                        previewImage: renderContext.previewImage
                    }
                });
            });
        }
...
export async function appendMessage({
    ...
    messageDiv.appendChild(createMainContentNode({ messageText, imageTagNodes, sender }));
    await finalizeRenderedMessage(messageDiv, { plainTextContent, reasoningContent, renderContext });
...
export async function updateMessageElement({
    messageElement,
    text,
    sender = null
}) {
    ...
    const resolvedSender = sender ||
        (messageElement.classList.contains('ai-message') ? 'ai' : 'user');
    const {
        plainTextContent,
        messageText,
        imageTagNodes,
    ...
    await finalizeRenderedMessage(messageElement, {
        plainTextContent,
        reasoningContent,
        renderContext
    });
