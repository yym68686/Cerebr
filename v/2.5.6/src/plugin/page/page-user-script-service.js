...
const MSG_I18N_EVENT = 'i18n.locale';
...
function createI18nBridge() {
    ...
    return {
        setLocale(locale) {
            currentLocale = normalizeString(locale);
            const payload = {
...
export function createPageUserScriptService({
    ...
    const i18nBridge = createI18nBridge();
    let unsubscribeLocaleBridge = null;

    const broadcastLocaleChange = (locale) => {
        Array.from(portsByKey.values()).forEach((portMeta) => {
            if (portMeta?.ready !== true) {
                return;
            }

            respondToPort(portMeta, {
                type: MSG_EVENT,
                event: MSG_I18N_EVENT,
                value: {
    ...
    const handleRuntimeMessage = async (message, sender) => {
        ...
        if (type === 'i18n.getLocale') {
            return {
                success: true,
                value: i18nBridge.getLocale(),
            };
        }

        if (type === 'i18n.getMessage') {
            const pluginId = normalizeString(message?.pluginId);
            const descriptor = descriptorsByPluginId.get(pluginId);
            ...
            const api = createPluginRuntimeI18nApi({
                plugin: {
                    id: pluginId,
                },
                manifest: descriptor.manifest || null,
                ...
                onLocaleChanged(callback, options = {}) {
                    return i18nBridge.onLocaleChanged(callback, options);
                },
            ...
            return {
                success: true,
                value: api.getMessage(
                    message?.key,
                    Array.isArray(message?.substitutions) ? message.substitutions : [],
                    message?.fallback || ''
                ),
            };
        }
    ...
    return {
        async start() {
            ...
            started = true;
            i18nBridge.setLocale(await localeStore.start());
            unsubscribeLocaleBridge = localeStore.subscribe(({ locale } = {}) => {
                i18nBridge.setLocale(locale);
        ...
        async stop() {
            ...
            unsubscribeLocaleBridge?.();
            unsubscribeLocaleBridge = null;
            localeStore.stop();
            await unregisterManagedScripts();
            portsByKey.clear();
