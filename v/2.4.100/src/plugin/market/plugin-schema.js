...
import { normalizePluginI18nConfig } from '../core/plugin-i18n.js';
...
function normalizeLocalizedText(value, key, fallback = '') {
    return {
        text: normalizeString(value, fallback),
...
function normalizeSelectionAction(value, id, index = 0) {
    ...
    const prompt = normalizeLocalizedText(
        value.prompt || value.text || value.promptTemplate,
        value.promptKey || value.textKey || value.promptTemplateKey
    );
    const label = normalizeLocalizedText(value.label, value.labelKey);
    if ((!prompt.text && !prompt.key) || (!label.text && !label.key)) {
        throw new Error(`Plugin "${id}" selectionActions[${index}] requires label/labelKey and prompt/promptKey`);
    }
    const title = normalizeLocalizedText(value.title, value.titleKey, label.text);
...
function normalizeShellExecute(value, id, locationLabel) {
    ...
    if (type === 'import_text' || type === 'insert_text' || type === 'set_draft') {
        const text = normalizeLocalizedText(
            execute.text || execute.prompt || execute.template,
            execute.textKey || execute.promptKey || execute.templateKey
        );
        if (!text.text && !text.key) {
            throw new Error(`Plugin "${id}" ${locationLabel} execute.type "${type}" requires text/textKey`);
        }
    ...
    if (type === 'show_toast') {
        const message = normalizeLocalizedText(
            execute.message || execute.text,
            execute.messageKey || execute.textKey
        );
        if (!message.text && !message.key) {
            throw new Error(`Plugin "${id}" ${locationLabel} execute.type "show_toast" requires message/messageKey`);
        }
...
function normalizeInputAction(value, id, index = 0) {
    ...
    if (!contributionId || ((!label.text && !label.key) && !icon)) {
        throw new Error(`Plugin "${id}" inputActions[${index}] requires id and label/labelKey or icon`);
    }
    const title = normalizeLocalizedText(value.title, value.titleKey, label.text || icon || contributionId);
    ...
    return {
        ...
        title: title.text,
        titleKey: title.key,
        variant: normalizeString(value.variant, icon && !(label.text || label.key) ? 'ghost' : 'soft'),
...
function normalizeMenuItem(value, id, index = 0) {
    ...
    const label = normalizeLocalizedText(value.label, value.labelKey);
    if (!contributionId || (!label.text && !label.key)) {
        throw new Error(`Plugin "${id}" menuItems[${index}] requires id and label/labelKey`);
    }
    const title = normalizeLocalizedText(value.title, value.titleKey, label.text);
...
function normalizeSlashCommand(value, id, index = 0) {
    ...
    const prompt = normalizeLocalizedText(
        value.prompt || value.text || value.template,
        value.promptKey || value.textKey || value.templateKey
    );
    if (!name || (!prompt.text && !prompt.key)) {
        throw new Error(`Plugin "${id}" slashCommands[${index}] requires name and prompt/promptKey`);
    }
    const label = normalizeLocalizedText(value.label, value.labelKey, name);
    const description = normalizeLocalizedText(value.description, value.descriptionKey);
...
export function validatePluginManifest(manifest, sourceUrl = '') {
    ...
    const normalized = {
        ...
        i18n: normalizePluginI18nConfig(manifest.i18n, {
            sourceUrl,
        }),
