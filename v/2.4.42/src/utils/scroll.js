...
function getInputOverlapPx() {
    ...
    const viewportBottom = (vv && Number.isFinite(vv.height))
        ? ((Number.isFinite(vv.offsetTop) ? vv.offsetTop : 0) + vv.height)
        : (window.innerHeight || document.documentElement.clientHeight || 0);
    return Math.max(0, viewportBottom - inputRect.top);
}
...
export function ensureChatElementVisible({
    ...
    requestAnimationFrame(() => {
        ...
        const vv = window.visualViewport;
        const viewportTop = (vv && Number.isFinite(vv.height))
            ? (Number.isFinite(vv.offsetTop) ? vv.offsetTop : 0)
            : 0;
        const viewportBottom = (vv && Number.isFinite(vv.height))
            ? (viewportTop + vv.height)
            : (window.innerHeight || document.documentElement.clientHeight || 0);

        const visibleTop = Math.max(viewportTop, containerRect.top) + marginPx;
        const visibleBottom = viewportBottom - bottomOffsetPx - marginPx;
