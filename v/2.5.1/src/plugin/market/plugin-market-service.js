...
function toInstalledViewModel(entry, state, appVersion, pluginPackage = null) {
    ...
    return {
        ...
        i18n: pluginPackage?.i18n && typeof pluginPackage.i18n === 'object'
            ? pluginPackage.i18n
            : (entry?.i18n && typeof entry.i18n === 'object' ? entry.i18n : null),
...
function toMarketplaceViewModel(entry, state, appVersion, pluginPackage = null) {
    const defaultInstalled = entry.kind === 'builtin' ? entry.defaultInstalled !== false : false;
    const installed = isPluginInstalled(state, entry.id, defaultInstalled);
    const record = getPluginRecord(state, entry.id) || {};
...
async function buildInstalledPackageMap(state) {
    ...
    const packageEntries = await Promise.all(
        installedPluginIds.map(async (pluginId) => {
            ...
            return [pluginId, pluginPackage];
        })
    );

    return new Map(packageEntries);
}
...
export async function getPluginMarketplaceModel() {
    ...
    const installedPackagesById = await buildInstalledPackageMap(effectiveState);
    ...
    return {
        ...
        installedItems: mergedEntries
            .map((entry) => toInstalledViewModel(
                entry,
                effectiveState,
                appVersion,
                installedPackagesById.get(entry.id) || null
            ))
            .filter(Boolean),
        marketplaceItems: mergedEntries
            .map((entry) => toMarketplaceViewModel(
                entry,
                effectiveState,
                appVersion,
                installedPackagesById.get(entry.id) || null
