...
function notifyLocaleWatchers() {
    const payload = {
        locale: normalizeString(state.currentLocale),
    };
    state.localeWatchers.forEach((callback) => {
        try {
            callback(payload);
        } catch (error) {
            console.warn('[Cerebr] User script locale watcher failed', error);
        }
    });
}
...
function handlePortMessage(message) {
    ...
    if (type === MSG_EVENT && normalizeString(message?.event) === 'i18n.locale') {
        state.currentLocale = normalizeString(message?.value?.locale);
        notifyLocaleWatchers();
        return;
    }
            ...
            onLocaleChanged(callback, { immediate = true } = {}) {
                ...
                if (immediate) {
                    if (state.currentLocale) {
                        ...
                        Promise.resolve(createRpcRequest('i18n.getLocale'))
                            .then((locale) => {
                                state.currentLocale = normalizeString(locale);
                                callback({
                                    locale: state.currentLocale,
                                });
                            })
