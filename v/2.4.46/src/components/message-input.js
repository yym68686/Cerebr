...
export function initMessageInput(config) {
    ...
    // iOS Safari：当手指在输入栏区域上下滑动时，避免触发页面级 rubber-banding/滚动条，
    // 将滚动手势重定向到聊天容器（#chat-container），确保输入栏保持吸底。
    (() => {
        ...
        let lastX = 0;
        let lastY = 0;
        let startedInMessageInput = false;

        const reset = () => {
            active = false;
            decided = false;
            shouldHandle = false;
            startedInMessageInput = false;
        };

        const isScrollableInput = () => {
            if (!messageInput) return false;
            return messageInput.scrollHeight > messageInput.clientHeight + 1;
        };

        const canStartFromTarget = (target) => {
            const el = target instanceof Element ? target : target?.parentElement;
            if (!el) return false;
            // Settings + menus should keep native gestures/clicks.
            if (el.closest('#settings-button, #settings-menu, #context-menu')) return false;
            // Buttons/links inside the input bar should keep native behavior.
            if (el.closest('button, a, select, input, textarea')) return false;
            return true;
        };
        ...
        inputContainer.addEventListener('touchstart', (e) => {
            ...
            if (!canStartFromTarget(e.target)) {
                reset();
                return;
            }
            ...
            startedInMessageInput = !!(e.target instanceof Element && e.target.closest('#message-input'));
        }, { passive: true });
        ...
        inputContainer.addEventListener('touchmove', (e) => {
            ...
            if (!decided) {
                if (Math.abs(totalDx) < MOVE_THRESHOLD_PX && Math.abs(totalDy) < MOVE_THRESHOLD_PX) {
                    lastX = x;
                    lastY = y;
                    return;
                }
                ...
                decided = true;
                const mostlyVertical = Math.abs(totalDy) >= Math.abs(totalDx);
                const chatScrollable = chatContainer.scrollHeight > chatContainer.clientHeight + 1;

                // 如果在输入框内部且输入框本身可滚动，则保留输入框内部滚动体验。
                if (startedInMessageInput && isScrollableInput()) {
                    shouldHandle = false;
                    return;
                }

                shouldHandle = mostlyVertical && chatScrollable;
            }
        ...
        inputContainer.addEventListener('touchend', reset, { passive: true });
        inputContainer.addEventListener('touchcancel', reset, { passive: true });
    })();
