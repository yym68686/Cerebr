...
async function getActiveTabForDiagnostics(tabId = null) {
...
async function getPluginRuntimeDiagnosticsSnapshot(tabId = null) {
  const hosts = {
    background: {
      host: 'background',
      available: true,
      diagnostics: backgroundPluginRuntime.getDiagnostics?.() || [],
      error: ''
    },
    page: {
      host: 'page',
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'GET_PLUGIN_RUNTIME_DIAGNOSTICS') {
    (async () => {
      try {
        const result = await getPluginRuntimeDiagnosticsSnapshot(message?.tabId);
        sendResponse(result);
      } catch (error) {
        sendResponse({
          success: false,
