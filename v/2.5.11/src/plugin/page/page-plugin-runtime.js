...
export function createPagePluginRuntime({
    ...
    const createI18nApi = (entry = {}) => createPluginRuntimeI18nApi(entry, {
        getLocale() {
    ...
    const createPageApi = (entry = {}, { api = {} } = {}) => {
        const permissions = createPermissionController(entry);

        return {
            getSelection() {
                permissions.assert('page:selection:read', ['page:selection']);
            ...
            clearSelection() {
                permissions.assert('page:selection:clear', ['page:selection']);
                try {
            ...
            query(selector) {
                permissions.assert('page:query', ['page:read']);
            ...
            getMessage(key, substitutions, fallback = '') {
                return api.i18n?.getMessage?.(key, substitutions, fallback)
                    ?? createI18nApi(entry).getMessage(key, substitutions, fallback);
            },
        };
    };
    ...
    const createSiteApi = (entry = {}) => {
        ...
        return {
            query(selector) {
                permissions.assert('site:query', ['site:read']);
                const normalizedSelector = normalizeString(selector);
                return normalizedSelector ? document.querySelector(normalizedSelector) : null;
            },
            queryAll(selector) {
            ...
            fill(selector, value) {
                permissions.assert('site:fill', ['site:write']);
                const normalizedSelector = normalizeString(selector);
    ...
    const createUiApi = (entry = {}) => {
        ...
        return {
            showAnchoredAction(config) {
                permissions.assert('ui:anchored-action', ['ui:mount']);
    ...
    const hostServiceRegistry = createHostServiceRegistry({
        ...
        page: {
            createApi: createPageApi,
        },
        site: {
            createApi: createSiteApi,
        },
        ui: {
    ...
    return {
        ...
        async start() {
            ...
            await runtimeController.pluginManager.notifyEvent?.('page.ready', {
                sticky: true,
            });
