...
export function initMessageInput(config) {
    ...
    const isIOS = () => {
        ...
        const isIpadOs = /Mac/i.test(platform) && navigator.maxTouchPoints > 1;
        return isAppleMobile || isIpadOs;
    };

    let lastIOSBackgroundClickAt = 0;
    const IOS_BACKGROUND_CLICK_DEDUP_MS = 50;

    const clickChatBackground = () => {
    ...
    const maybePreClickBackgroundForIOS = (event) => {
        if (!isIOS()) return;
        if (!event?.isTrusted) return;
        if (document.activeElement === messageInput) return;

        const now = performance.now();
        if (now - lastIOSBackgroundClickAt < IOS_BACKGROUND_CLICK_DEDUP_MS) return;
        lastIOSBackgroundClickAt = now;

        // iOS Safari: when a message bubble is in a sticky "tap-hover" state,
        // focusing the input can behave differently. Users expect one tap to work:
        // pre-trigger a background click (same as "tap empty chat area") before the real input click.
        clickChatBackground();
    };
    ...
    messageInput.addEventListener('pointerdown', (e) => {
        ...
        maybePreClickBackgroundForIOS(e);
