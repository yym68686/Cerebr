...
### 4. 当前已补上的 shell 宿主导航 API

为了把插件 UI 从“自己写模态框”迁到宿主原生结构，当前 runtime 已经补齐三组 shell API：

- `shell.setInputActions()` / `shell.onInputAction()`
  - 让宿主渲染输入框下方原生按钮，插件只处理点击事件。
- `shell.setMenuItems()` / `shell.onMenuAction()`
  - 让插件把入口接入 Cerebr 设置菜单的一层列表，而不是自己找 DOM 塞按钮。
- `shell.openPage()` / `shell.updatePage()` / `shell.closePage()` / `shell.onPageEvent()`
  - 让插件把自己的设置页、管理页挂到 Cerebr 宿主页框架中，复用返回按钮、标题区和页面容器。

这套 API 的目标不是让插件失去自由，而是把高频导航结构收口到宿主里：插件只负责页面内容，宿主负责页面壳、菜单层级、按钮风格和回退行为。
...
- 小型尾部按钮用 `shell.input.after`
- 整行工具栏或输入区增强面板用 `shell.input.row.after`
...
- `chat.onBeforeSend`
- `ui slot: shell.input.after` 或 `shell.input.row.after`
